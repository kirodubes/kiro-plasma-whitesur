...
Kiro's **Kiro WhiteSur** Plasma global theme — the polished macOS look (KDE Store #2 by
downloads), packaged as ready-to-pick Global Themes for the Kiro Plasma edition.

Everything ships under a `Kiro-` namespace, so it **coexists with the upstream WhiteSur
theme** — install the real `whitesur-kde-theme-git` alongside it without any file conflict.
